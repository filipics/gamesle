 ...
 * 
 * Mejoras en el modo diario:
 * - El botón de reinicio no funciona en modo diario.
 * - Si la partida ya terminó en modo diario, al cambiar a modo normal
 *   y volver a diario se restaura el estado final correctamente y se
 *   desactiva la entrada de teclado.
 ************************************************************/
...
// Aunque en esta versión no usamos guessedWords para detectar el fin,
// se podría seguir utilizando en una versión futura.
let guessedWords = [];
...
function loadDailyGameState() {
  ...
  // Si existe estado guardado y la fecha es hoy:
  if (savedGame && savedGame.lastPlayedDate === new Date().toDateString()) {
    ...
    // Restaurar el estado del tablero (cada celda tiene un <span>)
    ...
    // Restaura el flag de partida finalizada
...
function saveDailyGameState() {
  if (isDailyMode) {
    ...
    const gameState = {
      ...
      gameOver: gameOver, // Se guarda si la partida ya terminó
...
function handleKeyPress(key) {
  // Si la partida ya terminó, no se procesan nuevas teclas
  if (gameOver) return;
  key = key.toLowerCase();
  if (key === "enter") {
    if (currentCol === 5) {
...
function processWord(inputWord) {
  ...
  // Segunda pasada: letras presentes (amarillo) o ausentes
  for (let i = 0; i < 5; i++) {
    const letter = inputWord[i];
...
/* ==================== Reiniciar ==================== */
function resetGame() {
  // En modo diario, el botón de reinicio no hace nada
  if (isDailyMode) {
  ...
  generateGrid();
  generateKeyboard();
  // Reactivar teclas (en caso de haber sido deshabilitadas)
...
document.getElementById("modeToggle").addEventListener("click", function () {
  ...
  if (isDailyMode) {
    // Al cambiar a modo diario, reconstruimos grid y teclado
    generateGrid();
    generateKeyboard();
    const savedDailyWord = localStorage.getItem("dailyWord");
    if (savedDailyWord) {
    ...
    // Cargamos el estado diario (si existe)
    if (!loadDailyGameState()) {
      // Si no existe estado previo, iniciamos la partida diaria y la guardamos
      selectRandomWord();
      saveDailyGameState();
    }
    // Desactivar el botón de reinicio en modo diario
    document.getElementById("reset-game").disabled = true;
    return;
  } else {
    // En modo normal se reactivan las funciones
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Inicialmente, se inicia en modo normal
  selectRandomWord();
  resetGame();
  generateKeyboard();
  updateHistoryDisplay();
